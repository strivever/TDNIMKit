...
- (BOOL)activityIndicatorHidden
{
    if (!self.model.message.isReceivedMsg)
    {
        ...
        if ([self.model.message.localExt[@"deliveryState"] intValue] == 2) {
            return NO;
        } else {
            return YES;
        }
...
- (BOOL)unreadHidden {
    ...
    { //音频
        ...
        return (hideIcon || self.model.message.isOutgoingMsg || [self.model.message isPlayed]||[self.model.message.remoteExt[@"spokesman"] isEqualToString:@"d"]);
    }
    return YES;
}
