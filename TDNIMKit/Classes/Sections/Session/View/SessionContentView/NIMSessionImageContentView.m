...
- (void)refresh:(NIMMessageModel *)data
{
    ...
    BOOL isOutgoingMsg;
    if (self.model.message.remoteExt[@"spokesman"]) {
        isOutgoingMsg = [self.model.message.remoteExt[@"spokesman"] isEqualToString:@"d"];
    } else {
        isOutgoingMsg = self.model.message.isOutgoingMsg;
    }
    self.progressView.hidden     = isOutgoingMsg ? (self.model.message.deliveryState != NIMMessageDeliveryStateDelivering) : (self.model.message.attachmentDownloadState != NIMMessageAttachmentDownloadStateDownloading);
