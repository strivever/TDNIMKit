...
- (void)refresh:(NIMMessageModel *)data{
    ...
    NSInteger duration = (object.duration+500)/1000;
    self.durationLabel.text = [NSString stringWithFormat:@"%zd\"",duration < 1 ? 1 : duration];//四舍五入
...
- (void)layoutSubviews{
    ...
    if (self.model.message.isOutgoingMsg|| [self.model.message.remoteExt[@"spokesman"] isEqualToString:@"d"]) {
...
{
    if (self.model.message && self.model.message.attachmentDownloadState == NIMMessageAttachmentDownloadStateNeedDownload)
    {
        [[NIMSDK sharedSDK].chatManager fetchMessageAttachment:self.model.message error:nil];
    }
