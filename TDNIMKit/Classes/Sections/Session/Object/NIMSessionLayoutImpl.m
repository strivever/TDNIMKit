...
@property (nonatomic, assign) CGFloat beforeHeight;
...
- (void)beforeTableViewContentOffSizeHeight {
    
    _beforeHeight = self.tableView.contentSize.height;
...
- (void)adjustOffset:(NSInteger)row {
    ...
    //            [self.tableView setContentOffset:CGPointMake(self.tableView.contentOffset.x, self.tableView.contentOffset.y + 10) animated:NO];
    [self.tableView setContentOffset:CGPointMake(self.tableView.contentOffset.x, height - _beforeHeight) animated:NO];
...
- (void)adjustTableView
{
    ...
    if (self.tableView.tag == 10086) {
        if (superView.tag == 10086) {
            containerSafeHeight = self.tableView.superview.frame.size.height - safeAreaInsets.bottom - 58;
            rect.size.height = containerSafeHeight - self.inputView.toolBar.nim_height - 58 - 60;
        } else if (superView.tag == 10085) {
            containerSafeHeight = self.tableView.superview.frame.size.height - safeAreaInsets.bottom - 60;
            rect.size.height = containerSafeHeight - 60 ;
        } else {
            containerSafeHeight = self.tableView.superview.frame.size.height - safeAreaInsets.bottom - 58;
            rect.size.height = containerSafeHeight - self.inputView.toolBar.nim_height - 58 ;
        }
    } else {
    ...
    if (self.tableView.tag == 10086) {
        rect.origin.y    = rect.origin.y > 0 ? 58 : rect.origin.y + 58;
...
- (void)setupRefreshControl
{
    ...
    [self.refreshControl setValue:@(60) forKey:@"snappingHeight"];
