...
@property (nonatomic, copy) NSString *chat_key;

@property (nonatomic, strong) NSMutableDictionary *gcIdDictionary;

@property (nonatomic, strong) NSMutableArray *tempArray;
@property (nonatomic, strong) NIMMessage *tempMessage;
                         ...
                         config:(id<NIMSessionConfig>)sessionConfig
{
    if (self = [self init]) {
        ...
        _chat_key = [_sessionConfig getChatKey];
        ...
        _gcIdDictionary         = [NSMutableDictionary dictionary];
...
- (void)resetMessages:(void(^)(NSError *error)) handler
{
    ...
    self.gcIdDictionary         = [NSMutableDictionary dictionary];
    
    self.tempArray = @[].mutableCopy;
    self.tempMessage = nil;
    
    NSArray<NIMMessage *> *messages1 = [[[NIMSDK sharedSDK] conversationManager] messagesInSession:_currentSession
                                                                                           message:nil
                                                                                             limit:_messageLimit];
    
    if (_chat_key) {
        if (messages1.count == 0 && [self.dataProvider respondsToSelector:@selector(pullDown:handler:)])
        {
                ...
                NIMKit_Dispatch_Async_Main(^{
                    for (NIMMessage *message in messages) {
                        ...
                        if (![self modelIsExist:model]) {
                            [[NIMSDK sharedSDK].conversationManager saveMessage:message forSession:_currentSession completion:nil];
                        }
                    }
                    [wself appendMessageModels:[self modelsWithMessages:[self filterNIMMessageForCurrentPatientWithMwssages:messages]]];
                    if (handler) {
                        handler(error);
                    }
                });
        ...
        NSArray *filterArray = [self filterNIMMessageForCurrentPatientWithMwssages:messages1];
        if (filterArray.count > 0) {
            [self appendMessageModels:[self modelsWithMessages:filterArray]];
            if (handler) {
                handler(nil);
            }
        } else {
            [self getLocalMessagesWithMessage:(NIMMessage *)messages1.lastObject handle:handler];
            return;
        }
    } else {
        [self appendMessageModels:[self modelsWithMessages:messages1]];
        if (handler) {
            handler(nil);
        }
    }
...
- (void)getLocalMessagesWithMessage:(NIMMessage *)message handle:(void(^)(NSError *error)) handler{
    NSArray<NIMMessage *> *messages1 = [[[NIMSDK sharedSDK] conversationManager] messagesInSession:_currentSession
                                                                                           message:message
                                                                                             limit:_messageLimit];
    if (messages1.count == 0 && [self.dataProvider respondsToSelector:@selector(pullDown:handler:)])
    {
        [self getRemoteMessagesWithMessage:nil handle:handler];
        return;
    }
    NSArray *filterArray = [self filterNIMMessageForCurrentPatientWithMwssages:messages1];
    if (filterArray.count == 0 && ![message.messageId isEqualToString:(NIMMessage *)messages1.lastObject.messageId]) {
        [self getLocalMessagesWithMessage:(NIMMessage *)messages1.lastObject handle:handler];
...
- (void)getRemoteMessagesWithMessage:(NIMMessage *)message handle:(void(^)(NSError *error)) handler{
    __weak typeof(self) wself = self;
    
    [self.dataProvider pullDown:message handler:^(NSError *error, NSArray<NIMMessage *> *messages) {
//        NIMKit_Dispatch_Async_Main(^{
...
//过滤属于当前患者的聊天记录
- (NSMutableArray<NIMMessage *>*)filterNIMMessageForCurrentPatientWithMwssages:(NSArray *)messages {
    NSMutableArray <NIMMessage *>*arr = @[].mutableCopy;
    for (NIMMessage *hisMessage in messages) {
        if (hisMessage.remoteExt[@"chat_key"] && [hisMessage.remoteExt[@"chat_key"] isEqualToString:_chat_key] ) {
            if ([hisMessage.remoteExt[@"endType"] isEqualToString:@"120"] ||
                [hisMessage.remoteExt[@"endType"] isEqualToString:@"121"] ||
                 [hisMessage.remoteExt[@"endType"] isEqualToString:@"130"] ||
                 [hisMessage.remoteExt[@"endType"] isEqualToString:@"131"] ||
                 [hisMessage.remoteExt[@"endType"] isEqualToString:@"133"]) {
                
            } else {
                NIMMessage *transformMessage = [_sessionConfig transformMessageTypeWithMessage:hisMessage];
                if (transformMessage) {
                    [arr addObject:transformMessage];
                }
            }
        }
    }
...
{
    if ([_msgIdDict objectForKey:model.message.messageId] != nil || [_gcIdDictionary objectForKey:[NSString stringWithFormat:@"%@",model.message.remoteExt[@"gc_msg_id"]]]!= nil) {
        return YES;
    }
    return NO;
}
...
- (void)loadHistoryMessagesWithComplete:(void(^)(NSInteger index, NSArray *messages , NSError *error))handler
{
    ...
    if (_chat_key) {
        NSInteger index = 0;
        NIMMessage *currentMessage = [[NIMMessage alloc] init];
        currentMessage.timestamp = currentOldestMsg.message.timestamp - 0.001;
        NSArray *messages = [[[NIMSDK sharedSDK] conversationManager] messagesInSession:_currentSession
                                                                                message:currentMessage
                                                                                  limit:self.messageLimit];
        if (messages.count == 0 && [self.dataProvider respondsToSelector:@selector(pullDown:handler:)])
        {
            __weak typeof(self) wself = self;
            [self.dataProvider pullDown:currentOldestMsg.message handler:^(NSError *error, NSArray *messages) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    NSInteger index = [wself insertMessages:[self filterNIMMessageForCurrentPatientWithMwssages:messages]];
                    if (handler) {
        ...
        NSArray *filterArray = [self filterNIMMessageForCurrentPatientWithMwssages:messages];
        
        if (filterArray.count > 0) {
            NSInteger index = [self insertMessages:[self filterNIMMessageForCurrentPatientWithMwssages:messages]];
            if (handler) {
                handler(index,messages,nil);
            }
        } else {
            [self chwLoadHistoryMessagesWithMessage:messages.firstObject complete:handler];
        }
...
- (void)chwLoadHistoryMessagesWithMessage:(NIMMessage *)message complete:(void(^)(NSInteger index, NSArray *messages , NSError *error))handler{
    NSInteger index = 0;
    NIMMessage *currentMessage = [[NIMMessage alloc] init];
    currentMessage.timestamp = message.timestamp - 0.001;
    NSArray *messages = [[[NIMSDK sharedSDK] conversationManager] messagesInSession:_currentSession
                                                                            message:currentMessage
                                                                              limit:self.messageLimit];
    if (messages.count == 0 && [self.dataProvider respondsToSelector:@selector(pullDown:handler:)])
    ...
    NSArray *filterArray = [self filterNIMMessageForCurrentPatientWithMwssages:messages.reverseObjectEnumerator.allObjects];
...
- (NSArray*)deleteMessageModel:(NIMMessageModel *)msgModel
{
    ...
    if (delMsgIndex > -1) {
        ...
        if (msgModel.message.remoteExt[@"gc_msg_id"]) {
            [_gcIdDictionary removeObjectForKey:[NSString stringWithFormat:@"%@",msgModel.message.remoteExt[@"gc_msg_id"]]];
        }
...
- (NSArray<NSIndexPath *> *)deleteModels:(NSRange)range
{
    ...
    for (NIMMessageModel *model in models) {
        ...
        if (delMsgIndex > -1) {
            ...
            if (model.message.remoteExt[@"gc_msg_id"]) {
                [_gcIdDictionary removeObjectForKey:[NSString stringWithFormat:@"%@",model.message.remoteExt[@"gc_msg_id"]]];
...
- (void)insertMessage:(NIMMessage *)message{
    ...
    if (model.message.remoteExt[@"gc_msg_id"]) {
        [self.gcIdDictionary setObject:model forKey:[NSString stringWithFormat:@"%@",model.message.remoteExt[@"gc_msg_id"]]];
    }}
