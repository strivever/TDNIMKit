...
- (void)loadMessages:(void (^)(NSArray *messages, NSError *error))handler
{
    ...
    [self.dataSource loadHistoryMessagesWithComplete:^(NSInteger index, NSArray *messages, NSError *error) {
        if (messages.count) {
            ...
            NSInteger firstRow = [self findMessageIndex:messages.lastObject] - 1;
...
- (void)mediaPicturePressedWithVC:(UIViewController *)vc
{
    __weak typeof(self) weakSelf = self;
        ...
        switch (type) {
            case PHAssetMediaTypeImage:
            {
                for (UIImage *image in images) {
                    //                    [self sendImageMessage:image chatKey:chatKey];
                    if (vc && [vc isKindOfClass:NSClassFromString(@"NTESSessionViewController")]) {
                        [vc performSelector:@selector(sendImageMessage:) withObject:image];
                    } else if (vc && [vc isKindOfClass:NSClassFromString(@"GCNewSeverChatVC")]) {
                        [vc performSelector:@selector(sendImageMessage:) withObject:image];
                    }
...
- (void)mediaShootPressedWithVC:(UIViewController *)vc
{
    __weak typeof(self) weakSelf = self;
...
- (void)onRefresh
{
    [self.layout beforeTableViewContentOffSizeHeight];
    
    __weak typeof(self) wself = self;
    [self loadMessages:^(NSArray *messages, NSError *error) {
        [wself.layout layoutAfterRefresh];
        [wself.layout tableviewEnableScroll];
        if (messages.count) {
            NSInteger row = [self findMessageIndex:messages.lastObject];
            //            if (row <= 0) {
