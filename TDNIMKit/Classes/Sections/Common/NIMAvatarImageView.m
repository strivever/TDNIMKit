...
- (void)setAvatarByMessage:(NIMMessage *)message
{
    ...
    if (message.messageType == NIMMessageTypeRobot)
    {
     ...
     if (message.remoteExt[@"useraccid"]) {
        from = message.remoteExt[@"useraccid"];
    }
    if (!from)
    {
        from = message.from;
    ...
    NSString *headUrl;
    if ([info.avatarUrlString containsString:@"?"]) {
        headUrl = [info.avatarUrlString componentsSeparatedByString:@"?"].firstObject;
    } else {
        headUrl = info.avatarUrlString;
    }
    NSURL *url = headUrl ? [NSURL URLWithString:headUrl] : nil;
    [self nim_setImageWithURL:url placeholderImage:info.avatarImage];
}
